...
- [ ] Add explicit retry classification, exponential backoff with a cap, and a manual retry callback.
- [ ] Ensure 400/401/409 stop automatic retries and retain visible state.
- [ ] Ensure `dispose()` invalidates in-flight completions.
- [ ] Add tests for bounded retry, permanent failure, disposal, and recovery.

### Task 2: Make loads generation-safe

**Files:** `src/App.tsx`, new `src/lib/requestGeneration.ts`, tests for the generation helper.

- [ ] Use an `AbortController` per recap load.
- [ ] Ignore all stale completions after a newer load begins or the component unmounts.
- [ ] Cancel the previous queue before installing a new one.
- [ ] Add a simulation proving an older response cannot replace newer server state.
...
- [ ] Represent persisted amounts as validated non-negative integer minor units.
- [ ] Keep display formatting separate from storage representation.
- [ ] Reject floats, exponent notation, negative values, and unsafe integer overflow at the API boundary.
- [ ] Preserve a deterministic compatibility conversion for existing integer JSON data.
- [ ] Add precision and overflow tests.
...
- [ ] Accept and validate `Idempotency-Key` and `client_operation_id`.
- [ ] Persist request hash, response status, and response body for replay.
- [ ] Return the recorded response for identical replays and a conflict for mismatched replays.
- [ ] Keep revision conflicts explicit and non-retryable.
- [ ] Add schema readiness checks and test repeated requests.
...
- [ ] Replace raw WebAuthn exception messages with request-ID-backed generic errors.
- [ ] Apply bounded passkey verification rate limits.
- [ ] Define trusted proxy behavior instead of blindly trusting forwarded headers.
- [ ] Add a CI check that documented PackTally routes are clearly marked implemented or design-only.
...
- [ ] Cover initial-load failure, rapid month switching, stale responses, retry classification, money precision, and idempotent replay.
- [ ] Document that current legacy routes cannot satisfy the PackTally OpenAPI contract.
